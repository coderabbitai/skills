...
2. **Run CodeRabbit Review**
   ...
   - Preserve an explicitly requested `--deep` or legacy `--light` review policy; in CLI 0.8.0, `--deep` selects the GitHub PR review policy and takes precedence, while `--light` is a hidden alias for normal review
