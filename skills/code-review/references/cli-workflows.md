...
- When the installed binary's help lists `--clear`, `coderabbit review findings --clear --dir <path>` dismisses all retained findings for that review directory, current branch, and base branch. Use it only when the user asks to forget findings they fixed or rejected; it runs without another confirmation prompt. Other scopes and incremental checkpoints are preserved. Clearing history does not verify a fix or prove a clean review, and later reviews may report new findings. CLI 0.7.6 does not support this option.
